           ...
           bg-white border-r border-[#D4AF37]/20 flex flex-col"
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="{{ route('admin.shipping.index') }}"
                    class="{{ $linkBase }} {{ request()->routeIs('admin.shipping.*') ? $active : $idle }}">

                    <span class="absolute left-0 top-2 bottom-2 w-[3px] rounded-r"
                        style="background: {{ request()->routeIs('admin.shipping.*') ? '#D4AF37' : 'transparent' }};">
                    </span>

                    {{-- Icon: Truck / Shipping --}}
                    <svg class="h-5 w-5 shrink-0 {{ request()->routeIs('admin.shipping.*') ? 'text-[#8f6a10]' : 'text-gray-500 group-hover:text-[#8f6a10]' }}"
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
